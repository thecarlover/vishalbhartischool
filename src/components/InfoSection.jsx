  ...
  return (
    <div className="grid grid-cols-3 gap-4 px-6 py-9">
      {boxes.map((box, index) => (
        <motion.div
          key={index}
          className="relative group overflow-hidden rounded-lg shadow-lg aspect-w-1 aspect-h-1"
          initial={{ opacity: 0, y: 50 }} 
          animate={{ opacity: 1, y: 0 }} // Animate to full opacity and normal position
          transition={{ delay: index * 0.5, duration: 0.8 }} // Delay and duration for staggered effect
        >
          <img
            src={box.image}
            alt={box.title}
